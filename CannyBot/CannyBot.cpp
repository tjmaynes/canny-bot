...
double** transformationMatrix(std::string name_of_matrix, double** matrix, int rows, int columns, const int a, const double alpha, const int distance, double theta) {
	...
	for (int i = 0; i < rows; i++)
	{
		for (int j = 0; j < columns; j++)
		{
			...
			if (i == 0 && j == 1){
				temp = (-(std::sin(theta)) * std::cos(alpha));
				if (temp == -0.0)
					temp = 0.0;
				matrix[i][j] = temp;
			}
			if (i == 0 && j == 2){
				temp = (std::sin(theta) * std::sin(alpha));
				if (temp == -0.0)
					temp = 0.0;
				matrix[i][j] = temp;
			}
			if (i == 0 && j == 3){
				temp = (a * std::cos(theta));
				if (temp == -0.0)
					temp = 0.0;
				matrix[i][j] = temp;
			}
			...
			if (i == 1 && j == 1){
				temp = (std::cos(theta) * std::cos(alpha));
				if (temp == -0.0)
					temp = 0.0;
				matrix[i][j] = temp;
			}
			if (i == 1 && j == 2){
				temp = (-(std::cos(theta)) * std::sin(alpha));
				if (temp == -0.0)
					temp = 0.0;
				matrix[i][j] = temp;
			}
			if (i == 1 && j == 3){
				temp = (a * std::sin(theta));
				if (temp == -0.0)
					temp = 0.0;
				matrix[i][j] = temp;
			}
